...
consent screen, scopes, revocable per-app. The resulting `xoxb-` token, however, is
then used as a **long-lived bearer token**, so it needs API-key-grade handling: env
var, never committed, rotated on exposure.
...
Socket Mode is the practical choice for a stack that lives behind a laptop firewall.
...
| Mention creates issue, nothing runs | Daemon not running — see `01_multica_setup.md` |
