...
That last point is the one that matters most: **it is the natural fit for the
sandbox/VM task.** You can put the agent where the code and the risk are, rather
than on your laptop.
...
on loop speed. For editing code you are actively reading, the editor integration is
genuinely better — the free context and the per-hunk diff review are hard to
match in a TUI.
...
Same posture as everywhere else, and more urgent because the tool runs shell
commands by design:
...
**Where to use it:** test-driven fixes, mechanical refactors, working on remote
machines, and anywhere the code should not leave a controlled environment.

**Where not to:** exploratory design work, and any repository you have not read
and do not trust.
