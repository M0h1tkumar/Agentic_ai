...
> **Status:** installation reference. Steps described plainly; steps not
> personally verified end-to-end are marked *(unverified)*.
...
- `.env` now holds a provider key. Confirm it is in `.gitignore` before your first
  commit — leaked static keys are the worst kind of credential exposure.
...
the issue just sits there.** This is confusing until you understand the architecture in
§1.
...
each other by service name *(unverified; this is the design followed from the
